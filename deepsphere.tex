...
In particular, the graph Fourier transform is defined as the projection on the graph Laplacian eigenvectors and the graph convolution as a multiplication in the Fourier domain. 
It turns out that graph convolution can also be performed directly in the vertex domain, i.e. using the pixel values on the sphere~\cite{hammond2011wavelets}.
...
The current implementation of DeepSphere relies on the Equal Area isoLatitude Pixelisation (HEALPix)~\citep{gorski2005healpix}, a popular sampling used in cosmology and astrophysics. See \citet{perraudin2018deepsphere} for details.
...
This should not be neglected since, as we shall see in the next section, the better the manifold representation, the closer to rotational equivariant the graph convolution will be. 
