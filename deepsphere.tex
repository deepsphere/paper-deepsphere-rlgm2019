...
Moreover, graph convolutions have a low computational cost of $\bO(N_{pix})$, where $N_{pix}$ is the number of considered pixels.
Methods based on proper spherical convolutions, like \citet{cohen2018sphericalcnn} and \citet{esteves2017sphericalcnn}, costs $\bO(N_{pix}^{2/3})$.
...
It turns out that the graph convolution can be accelerated by being performed directly in the vertex domain, i.e., using the pixel values on the sphere~\citep{hammond2011wavelets}.
...
See an example architecture in \figref{architecture}.
...
\begin{minipage}[c]{.21\textwidth}
	...
	\captionof{figure}{circle $\mathcal{C}^1$ (green), regular samples and graph vertices (red), graph edges (blue).}
	\label{fig:ring}
\end{minipage}
\hfill
...
\begin{figure}
	...
	\caption{Correspondence between the subspaces spanned by the graph Fourier basis and the spherical harmonics.
		...
		The graph Fourier basis agree at higher frequencies as the resolution ($N_{side}$) increases.}
...
We utilize two different networks: the ``CNN variant'', where the convolutional layers are followed by a dense layer, and the ``FCN variant'', where the convolutional layers are followed by global average pooling.
...
Overall, DeepSphere performs best with a gap that widens as the problem becomes more difficult.
...
As our task is rotation invariant (due to the cosmological principle of homogeneity and isotropy), the FCN variant outperforms the CNN variant.
The 2D ConvNet fairs in-between the SVMs and DeepSphere.
We believe that its relatively low performance is due to its lack of rotation equivariance.
This architecture does not exploit the rotational symmetries of the task.
Moreover, the projection of spherical maps to flat images distorts the geometry, and the NN has to learn to compensate for it.%, which comparatively requires more training data.
...
% \todo{Pierre, Cardoso, Tomek? -> Tomek should be an author...}
